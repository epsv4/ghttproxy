...
function resetMyPlane(){
	...
	document.onmousemove = function(e) {
		e = e || window.event;
		if(!isPause) {
			movePlane({clientX:e.clientX, clientY:e.clientY});
		}
	}
	let eventMove = function(e){
		...
		e.preventDefault();
		if(!isPause) {
			movePlane({clientX:touchE.clientX, clientY:touchE.clientY});
		}
	}
...
function myPlaneBomb() {
	...
	isDie = true;
	clearInterval(times);
	post("/ajax", JSON.stringify({score:scores}))
}
//生成分数
...
function createPlane(level, plane, oTop, oLeft) {
	...
	myPlane.style.width = int2str(plane[level].width);
	myPlane.style.height = int2str(plane[level].height);
...
function post(url, data, callBack=null, breadDown=null){
	var xhr;
	if(window.XMLHttpRequest) {
		xhr = new XMLHttpRequest();
	} else {
